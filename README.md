...
각 결과의 식당 이름 옆 **위치 보기**를 누르면 목록을 벗어나지 않고 앱 안의 지도 패널에서 위치를 확인합니다. 좌표가 없는 지역 공공데이터 결과는 사용자가 패널을 열었을 때만 식당명과 주소로 좌표를 보완합니다. 실제 길찾기가 필요할 때에만 패널의 Google 지도 또는 OpenStreetMap 버튼을 사용합니다. 별도 Google API 키는 사용하지 않습니다.
