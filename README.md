...
- `맛집 등록`은 로그인 없이 바로 동작하며, 등록 직후 검색 결과에 표시됩니다. 등록한 맛집은 주소 또는 좌표가 검색 지역과 일치하면 지역 검색에 포함됩니다. 업체명(예: `일품청국장`)은 어느 검색창에 입력해도 다시 찾을 수 있습니다. 브라우저에 저장되는 임의의 기기 식별자로 등록 목록을 분리하며, 같은 기기에서만 다시 확인할 수 있습니다. 브라우저 데이터를 지우면 이 기기 등록 목록에 다시 접근할 수 없으므로 주의해 주세요.
