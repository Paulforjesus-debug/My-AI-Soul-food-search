...
현재 버전에서는 **도시·지역·동네 입력 → 탐색하기**를 누르면 입력 위치를 좌표로 변환한 뒤 한국관광공사, 적용 가능한 지역 공공데이터, OpenStreetMap을 함께 확인합니다. 현재 위치 권한은 선택 기능입니다. 대구에서는 대구광역시 대구푸드 공식 맛집 데이터가 자동으로 추가되며, 이름·주소·전화번호 또는 근접 좌표가 일치하는 결과는 하나로 합쳐 복수 출처 신뢰도를 높입니다. 요청은 사용자 동작 때만 발생하며 10초의 재검색 간격을 둡니다. 지역 좌표 변환은 24시간 캐시하고 공개 OSM 정책에 맞춰 초당 1회 이하로 제한합니다.
